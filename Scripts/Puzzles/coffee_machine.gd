...
func _on_interact():
	if (GameController.phase == GameConstants.PHASES.PREP):
		GameController.launch_puzzle("res://Scenes/Prep_puzzels/prep_coffee_machine.tscn")
	else:
		GameController.launch_puzzle("res://Scenes/Service_games/Service_Coffee_Machine.tscn")
